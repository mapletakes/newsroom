// Token-authed segment list, for the browser extension's "add to segment" menu.
...
export async function GET(req: NextRequest) {
  ...
  const segments = (data || []).map((s) => ({ id: s.id, name: s.name }));
  return NextResponse.json({ ok: true, segments }, { headers: CORS });
}

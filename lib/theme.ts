...
export const PALETTES: Record<string, Palette> = {
  ...
  // Built the same way, but from ACCENT_HUES_COLORBLIND instead of
  // ACCENT_HUES — see the note there for why. Everything else (contrast
  // guarantee, generation, drift test) is identical to broadsheet/midnight/ash.
  colorblind: { ink: '#121211', paper: '#f4f4f3', rust: '#915222', ochre: '#7c5e1d', moss: '#1a7059', slate: '#226891' },
};
...
export const PALETTE_META: Record<string, { label: string; icon: string }> = {
  ...
  colorblind: { label: 'Colorblind-friendly', icon: 'themeColorblind' },
};
...
export type AccentToken = Exclude<PaletteToken, 'ink' | 'paper'>;
export type AccentHues = Record<AccentToken, number>;

export const ACCENT_HUES: AccentHues = {
  rust: 14,
  ochre: 42,
  moss: 122,
  slate: 214,
...
/**
 * An alternate hue set for the "Colorblind-friendly" palette, sourced from
 * Okabe & Ito's Color Universal Design palette (2002; the reference behind
 * Nature journals' figure-colour guidance) rather than picked by eye.
 *
 * Only `moss` moves meaningfully — from 122° (pure green) to 164° (bluish-
 * green). That's the one change that matters: protanopia and deuteranopia
 * both confound colours along the red-green opponent channel, and 14°
 * (rust) and 122° both sit inside that confusable range, only 108° apart.
 * Okabe-Ito's "bluish green" is deliberately clear of it — 164° is well into
 * the range still discriminable via the blue-yellow channel CVD leaves
 * intact, taking the rust/moss separation to 138°. `ochre` barely moves
 * (42°→41°) because amber was never the problem; `slate` (214°→202°) tracks
 * Okabe-Ito's blue.
 *
 * Contrast is guaranteed exactly like every other palette — this only
 * changes WHICH hues are solved for, not whether the solve succeeds.
 */
export const ACCENT_HUES_COLORBLIND: AccentHues = {
  rust: 26, // vermillion
  ochre: 41, // orange
  moss: 164, // bluish green — the meaningful shift
  slate: 202, // blue
};
...
/** A complete, contrast-guaranteed palette from a recipe. `hues` defaults to
 *  the standard four; the colorblind-friendly palette is generated by the
 *  same function with ACCENT_HUES_COLORBLIND instead — the guarantee and the
 *  generation logic don't change, only which hues get solved for. */
export function derivePalette(recipe: PaletteRecipe, hues: AccentHues = ACCENT_HUES): Palette {
  ...
  for (const [token, hueDeg] of Object.entries(hues)) {
...
export type AppTheme = {
  ...
  /**
   * Tokens explicitly pinned to a hex value from the builder's swatches,
   * overriding what `recipe` would otherwise derive for them. This is the
   * escape hatch out of the guarantee, not a hole in it: a streamer who
   * wants an exact brand colour on one swatch can have it, but only that
   * swatch opts out — the rest keep deriving normally, and the UI keeps
   * showing that swatch's real, currently-measured ratio rather than hiding
   * that it may now read below the bar.
   */
...
export function sanitizeAppTheme(raw: unknown): AppTheme {
  ...
  const overrides = pickColors(o.overrides, PALETTE_TOKENS);
  return {

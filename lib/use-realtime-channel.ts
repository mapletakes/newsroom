...
/**
 * Subscribe to "changed" broadcasts on a Supabase Realtime topic. Calls
 * `onChange` whenever the server pings it. Callers pair this with a slow poll
 * as a backstop.
 *
 ...
 *
 * Pulled out from the queue-specific version so every topic (queue,
 * questions, …) shares this one design rather than re-deriving it — a topic
 * is just a string, and the reconnect trade-off above applies equally to all
 * of them.
 */
export function useRealtimeChannel(topic: string | null, onChange: () => void) {
